...
sub verifica_facturas_id {
    ...
    $sql = sprintf "SELECT COUNT(*) FROM pfacturas WHERE fecha='%s' AND factura='%s' AND proveedor='15002' AND recepcion IS NOT NULL AND recepcion>0 ", $invoicedate, $invoiceid;
    $sql = $sql . $andquery;
